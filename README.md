# 📱 Homepage-Techniker App

**Die eigene App der Koblenzer Puppenspiele** — Homepage-Bearbeitung per Sprache, KI-Assistent und Canva-artiger Editor — direkt auf deinem Smartphone.

---

## ⬇️ Download (neueste Version)

> 👉 **Direkt-Download:** [Homepage-Techniker.apk herunterladen](https://github.com/sebastianmoschek-hash/homepage-techniker-app/releases/latest/download/Homepage-Techniker.apk)

**Version:** v0.11.0 (KI-Fix) · **Größe:** ~280 MB

---

## 📲 Installation (Android)

1. **Öffne diese Seite auf deinem Handy** (z. B. über den QR-Code / Link aus der Nachricht).
2. Tippe auf **„Homepage-Techniker.apk herunterladen“**.
3. Der Download startet — das dauert je nach Internetgeschwindigkeit 1–5 Minuten.
4. Öffne die **heruntergeladene Datei** (Benachrichtigung „Download abgeschlossen“).
5. Falls gefragt wird: **„Unbekannte Quellen erlauben“** → erlauben (nur für diese Installation).
6. Tippe auf **Installieren** (bei vorhandener Version: **Aktualisieren**).
7. Fertig! 🎉

---

## ✨ Was die App kann

- 🤖 **KI-Assistent** — stellt Fragen zur Homepage, beantwortet sie und hilft beim Bearbeiten
- 🎨 **Canva-artiger Editor** — Texte antippen, Drag & Drop, Bilder austauschen
- 🗣️ **Sprachsteuerung** — „Mach den Button größer“, „Was siehst du gerade?“ (offline)
- 🖼️ **KI-Freistellen / Bilder generieren**
- 📺 **Bildschirmfreigabe mit KI** — der Assistent sieht, was du siehst
- 💾 **Automatisches Speichern + Undo/Redo** (48 Std. Verlauf)

---

## ❓ Hilfe

Bei Fragen zur Installation oder zur App: Einfach dem Betreuer-Team schreiben.

---

*© Koblenzer Puppenspiele*